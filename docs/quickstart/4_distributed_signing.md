...
- A `tsa` API key for RFC 3161 timestamping. Configure it with `trufo set-api-key tsa <your-api-key>` or `TRUFO_TSA_API_KEY`.
  (To try the timestamping flow before you have a key, the free test endpoint `https://test.tsa.trufo.ai/` accepts keyless requests; its tokens are not production-trusted. The production TSA at `https://tsa.trufo.ai/` is what the signing helpers use — see [tca_ca.md](../api/tca_ca.md) for the RFC 3161 reference.)
